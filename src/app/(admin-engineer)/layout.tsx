          ...
          <nav className="space-y-1.5 px-3">
            ...
            {(role === 'Social Media Director' || role === 'Customer Care' || role === 'Super Admin') && (
              <>
                ...
                <Link href="/admin-engineer/leads" onClick={closeMenu} className={linkClass('/admin/leads')}>
            ...
            {(role === 'Customer Care' || role === 'Super Admin') && (
              <>
                <Link href="/admin-engineer/tickets" onClick={closeMenu} className={linkClass('/admin/tickets')}>
                  <CheckSquare className={iconClass('/admin/tickets')} />
                  Support Tickets
                </Link>
                <Link href="/admin-engineer/inspections" onClick={closeMenu} className={linkClass('/admin/inspections')}>
            ...
            {(role === 'Social Media Director' || role === 'Super Admin') && (
              <>
                ...
                <Link href="/admin-engineer/campaigns" onClick={closeMenu} className={linkClass('/admin/campaigns')}>
                  <Megaphone className={iconClass('/admin/campaigns')} />
                  Campaign Manager
                </Link>
                <Link href="/admin-engineer/announcements" onClick={closeMenu} className={linkClass('/admin/operations/announcements')}>
            ...
            {(role === 'Secretary' || role === 'Chairman' || role === 'Super Admin') && (
              <>
                ...
                {(role === 'Secretary' || role === 'Super Admin') && (
                  <Link href="/admin-engineer/reservations" onClick={closeMenu} className={linkClass('/admin/reservations')}>
                    <Wallet className={iconClass('/admin/reservations')} />
                    Reservations
                  </Link>
                )}
                <Link href="/admin-engineer/payments" onClick={closeMenu} className={linkClass('/admin/payments')}>
                  <Wallet className={iconClass('/admin/payments')} />
                  Payments & Ledger
                </Link>
                {(role === 'Chairman' || role === 'Super Admin') && (
                  <Link href="/admin-engineer/approvals" onClick={closeMenu} className={linkClass('/admin/approvals')}>
            ...
            {(role === 'Director' || role === 'Chairman' || role === 'Super Admin') && (
              <>
                ...
                <Link href="/admin-engineer/customers" onClick={closeMenu} className={linkClass('/admin/customers')}>
            ...
            {(role === 'Admin Engineer' || role === 'Super Admin' || role === 'Director' || role === 'Chairman') && (
              <>
                ...
                <Link href="/admin-engineer/projects" onClick={closeMenu} className={linkClass('/admin/projects')}>
                  <FolderGit2 className={iconClass('/admin/projects')} />
                  Estate Projects
                </Link>
                {(role === 'Admin Engineer' || role === 'Super Admin') && (
                  <>
                    <Link href="/admin-engineer/listings" onClick={closeMenu} className={linkClass('/admin/listings')}>
                      <MapPin className={iconClass('/admin/listings')} />
                      Website Listings
                    </Link>
                    <Link href="/admin-engineer/property-submissions" onClick={closeMenu} className={linkClass('/admin/property-submissions')}>
            ...
            {(role === 'Admin Engineer' || role === 'Super Admin' || role === 'Chairman') && (
              <>
                ...
                <Link href="/admin-engineer/logs" onClick={closeMenu} className={linkClass('/admin/logs')}>
                  <Activity className={iconClass('/admin/logs')} />
                  Global Audit Logs
                </Link>
                {(role === 'Admin Engineer' || role === 'Super Admin') && (
                  <>
                    <Link href="/admin-engineer/tasks" onClick={closeMenu} className={linkClass('/admin/tasks')}>
                      <CheckSquare className={iconClass('/admin/tasks')} />
                      System Tasks
                    </Link>
                    <Link href="/admin-engineer/settings" onClick={closeMenu} className={linkClass('/admin/settings')}>
      ...
      <main className="flex-1 flex flex-col h-full overflow-hidden w-full relative">
        <header className="bg-white/80 backdrop-blur-md shadow-sm border-b border-gray-200 h-16 flex items-center justify-between px-4 lg:px-8 z-30 sticky top-0">
          ...
          <div className="flex items-center gap-3 sm:gap-5">
            <Link href="/admin-engineer/leads">
